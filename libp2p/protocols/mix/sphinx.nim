...
proc computeDelta(s: seq[seq[byte]], msg: Message): Result[seq[byte], string] =
  ...
  for i in countdown(s.len - 1, 0):
    let cipher = Lioness.init(deriveLionessKey(s[i]))
...
proc useSURB*(surb: SURB, msg: Message): SphinxPacket =
  var delta = msg.serialize()
  let cipher = Lioness.init(deriveLionessKey(surb.key))
  doAssert cipher.encrypt(delta).isOk,
    "LIONESS encrypt failed: payload below minimum block size"
...
proc processReply*(
  ...
  let cipher = Lioness.init(deriveLionessKey(key))
...
proc processSphinxPacket*(
  ...
  let cipher = Lioness.init(deriveLionessKey(sBytes))
